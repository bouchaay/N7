...
module NatSet (B : Base) = struct
  ...
  (* Instanciation du module Conversion *)
  module C = Conversion (B)
  
  (* Instanciation du module Arbre *)
  module A = Arbre (B)

  (* type natset représentant un ensemble dans la base de données *)
  type natset = A.arbre
  ...
  let appartient entier arbre = A.appartient (C.decompose entier) arbre

  (* ajouter : int -> natset -> natset *)
  (* ajoute un entier x à un ensemble e *)
  let ajouter entier arbre = A.ajouter (C.decompose entier) arbre

  (* retirer : int -> natset -> natset *)
  (* retire un entier x d'un ensemble e *)
  let retirer entier arbre = A.retirer (C.decompose entier) arbre
