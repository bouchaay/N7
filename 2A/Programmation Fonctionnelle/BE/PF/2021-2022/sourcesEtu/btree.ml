...
let rec height tr = match tr with 
    | Leaf _0 -> 0
    | Node (subtree_left, subtree_right) -> 
        max ((height subtree_left) + 1) ((height subtree_right) + 1)
...
let rec num_nodes tr = match tr with 
    | Leaf _ -> 1
    | (Node (subtree_left, subtree_right)) -> 
        num_nodes(subtree_left) + num_nodes(subtree_right) + 1
...
let rec num_values tr = match tr with 
    ...
    | Leaf _  -> 1
    | Node (subtree_left, subtree_right) -> 
        num_values(subtree_left) + num_values(subtree_right) 
 ...
 *)
let rec search tr addrl = match addrl, tr with 
    | [], Leaf c -> c
    | [_], Leaf '0' -> '0'
    | t::q, Node(subtree_left, subtree_right) -> 
        if (t = 0) then search (subtree_left) q
        else search (subtree_right) q
    | _ -> raise (Failure "") 
