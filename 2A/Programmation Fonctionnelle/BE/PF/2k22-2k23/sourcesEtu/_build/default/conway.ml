...
let max = fun l -> match l with [] -> failwith "Liste vide !" 
  | h::_ -> List.fold_left (fun acc x -> if (acc > x) then acc else x) h l
...
(* Résultat :  l'élément le plus grand de la liste *)
let max_max = fun l -> max (List.flatten l)
...
let suivant = fun _ -> assert false
