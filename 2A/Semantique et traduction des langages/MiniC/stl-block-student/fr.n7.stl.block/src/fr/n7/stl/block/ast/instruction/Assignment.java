...
public class Assignment implements Instruction, Expression {
	...
	public boolean collectAndBackwardResolve(HierarchicalScope<Declaration> _scope) {
		boolean _ok = this.assignable.collectAndBackwardResolve(_scope) && this.value.collectAndBackwardResolve(_scope);
		return _ok;
	}
	...
	public boolean fullResolve(HierarchicalScope<Declaration> _scope) {
		boolean _ok = this.assignable.fullResolve(_scope) && this.value.fullResolve(_scope);
		return _ok;
	}
	...
	public boolean checkType() {
		if (this.assignable.getType().compatibleWith(this.value.getType())) {
			...
			System.err.println("Type error: incompatible types for assignment (assignment)");
			return false;
		}
	}
	...
	public int allocateMemory(Register _register, int _offset) {
		...
		return _offset + this.assignable.getType().length();
	}
