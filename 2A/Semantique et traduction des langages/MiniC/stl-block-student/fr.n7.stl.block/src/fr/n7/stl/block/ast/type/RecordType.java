...
public class RecordType implements Type, Declaration, Scope<FieldDeclaration> {
	...
	public boolean equalsTo(Type _other) {
		...
		if (_other instanceof RecordType) {
			RecordType _otherRecord = (RecordType) _other;
			if (this.fields.size() != _otherRecord.fields.size()) {
				return false;
			}
			for (FieldDeclaration f : this.fields) {
				boolean found = true;
				for (FieldDeclaration otherField : _otherRecord.fields) {
					if (f.getName().equals(_otherRecord.getName()) && f.getType().equalsTo(otherField.getType())){
						found = true;
						break;
					}
				}
	...
	public boolean compatibleWith(Type _other) {
	    if(_other instanceof RecordType){
	        ...
	        for (FieldDeclaration f : this.fields) {
	            for (FieldDeclaration otherField : _otherRecord.fields) {
	                found = found && f.getType().compatibleWith(otherField.getType());
	            }
	        }
	        return found;
	    } else {
	...
	public Type merge(Type _other) {
		...
		if (_other instanceof RecordType) {
			RecordType _otherRecord = (RecordType) _other;
			RecordType _result = new RecordType(this.name);
			for (FieldDeclaration f : this.fields) {
				for (FieldDeclaration otherField : _otherRecord.fields) {
					if (f.getName().equals(otherField.getName())) {
						_result.add(new FieldDeclaration(f.getName(), f.getType().merge(otherField.getType())));
					}
				}
			}
			return _result;
