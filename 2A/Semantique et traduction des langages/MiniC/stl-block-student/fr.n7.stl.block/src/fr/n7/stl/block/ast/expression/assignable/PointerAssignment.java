...
public class PointerAssignment extends AbstractPointer implements AssignableExpression {
	...
	public Fragment getCode(TAMFactory _factory) {
		...
		VariableAssignment pointerAssign = (VariableAssignment) this.pointer;
		fragment.add(_factory.createLoad(pointerAssign.declaration.getRegister(), pointerAssign.declaration.getOffset(), this.pointer.getType().length()));
		fragment.add(_factory.createStoreI(this.pointer.getType().length()));
		return fragment;
	}
