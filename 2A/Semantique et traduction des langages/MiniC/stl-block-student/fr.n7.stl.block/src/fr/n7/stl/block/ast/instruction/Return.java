...
public class Return implements Instruction {
	...
	public boolean checkType() {
		if(this.value.getType().compatibleWith(AtomicType.ErrorType) || this.value.getType().compatibleWith(AtomicType.VoidType)) {
			System.err.println("The return value is not compatible with the function return type. (Return)");
			return false;
		} else {
			return true;
		}
	...
	public Fragment getCode(TAMFactory _factory) {
		...
		fragment.add(_factory.createReturn(this.value.getType().length(), 0));
		return fragment;
	}
