...
public class VariableDeclaration implements Declaration, Instruction {
	...
	public boolean checkType() {
		if (this.type.compatibleWith(this.value.getType()) || this.value.getType().compatibleWith(this.type)) {
			return true;
		} else {
			System.err.println("Type Error: Type mismatch between the declared variable and the initial value. (VariableDeclaration)");
			return false;
		}
	}
	...
	public int allocateMemory(Register _register, int _offset) {
		...
		this.offset = _offset + this.type.length();
		return this.offset;
	}
