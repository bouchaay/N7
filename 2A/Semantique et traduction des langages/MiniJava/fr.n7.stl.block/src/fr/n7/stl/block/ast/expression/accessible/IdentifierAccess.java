...
public class IdentifierAccess extends AbstractIdentifier implements AccessibleExpression {
	...
	public boolean collectAndBackwardResolve(HierarchicalScope<Declaration> _scope) {
		if (this.name.equals("this") && SymbolTable.classe != null) {
			this.expression = new ThisAccess(SymbolTable.classe);
			return true;
		}
	...
	public boolean fullResolve(HierarchicalScope<Declaration> _scope) {
		...
		if (this.expression == null) {
			...
			if (((HierarchicalScope<Declaration>) _scope).knows(this.name)) {
				...
				if (_declaration instanceof VariableDeclaration) {
					// throw new SemanticsUndefinedException( "Collect and partial resolve have
					// probably not been implemented correctly. The identifier " + this.name + "
					// should have not been resolved previously.");
					this.expression = new VariableAccess((VariableDeclaration) _declaration);
					return true;
				} else {
					/* These kinds are handled by full resolve. */
