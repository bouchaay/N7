...
public abstract class AbstractField implements Expression {
	...
	public boolean fullResolve(HierarchicalScope<Declaration> _scope) {
		if (this.record.toString().equals("this") && SymbolTable.classe != null) {
			if ((SymbolTable.classe.getElementsTable().knows(this.name))) {
				Declaration _declaration = SymbolTable.classe.getElementsTable().get(this.name);
		...
		if (_type instanceof Instance) {
			...
			if (d instanceof NormalClass) {
				if (((NormalClass) d).getElementsTable().contains(this.name)) {
					Declaration dec = ((NormalClass) d).getElementsTable().get(name);
