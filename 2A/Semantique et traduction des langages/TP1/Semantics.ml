...
let rec value_of_expr expr env =
  ...
  and
    ...
    let value =
      (value_of_expr bvalue env) (* Evaluation de la valeur *)
    in
    match value with
    | (ErrorValue _) as result -> result (* Propagation de l'erreur *)
    | _ ->
       (let newenv =
          (ident,value)::env (* Ajout de la valeur dans l'environnement *)
        in
        (value_of_expr bin newenv)) (* Evaluation de la suite avec le nouvel environnement *)
  ...
  and
    ...
    let value =
      (value_of_expr cond env) (* Evaluation de la condition *)
    in
    match value with
    | (ErrorValue _) as result -> result (* Propagation de l'erreur *)
    | (BooleanValue true) -> (value_of_expr bthen env) (* Evaluation du then *)
    | (BooleanValue false) -> (value_of_expr belse env) (* Evaluation du else *)
    | _ -> (ErrorValue TypeMismatchError)
  ...
  and
    ...
    ruleFunction env expr = FrozenValue (expr,env)
