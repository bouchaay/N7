	...
	begin
      ...
      Max := 0;
		for I in 1..Taille loop
		    ...
		    if not (Cle_Presente(Sda =>LISTE, Cle => Nombre)) then
			    Enregistrer(Sda => LISTE, Cle => Nombre, Donnee => 1);
		    else
			    Enregistrer(Sda => LISTE, Cle => Nombre, Donnee => La_Donnee(Sda => LISTE, Cle => Nombre) + 1);
		    end if;
		end loop;
		for i in 1..Borne loop
			if La_Donnee (Sda => LISTE, Cle => i) < Min then
				Min := La_Donnee (Sda => LISTE, Cle => i);
			else Null;
			end if;
			if La_Donnee (Sda => LISTE, Cle => i) > Max then
				Max := La_Donnee (Sda => LISTE, Cle => i);
			else Null;
			end if;
